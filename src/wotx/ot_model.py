...
class OTModel:
    ...
    **kwargs : dict
    ...
    def __init__(self, matrix, day_field='day', covariate_field='covariate',
                growth_rate_field='cell_growth_rate',growth_average_field='growth_average',
                mask_matrix= 'mask_matrix',**kwargs):
        self.matrix = matrix
        ...
        self.ot_config = {'local_pca': 30, 'growth_iters': 1, 'epsilon': 0.05, 'lambda1': 1, 'lambda2': 50,'delta':0.0001,
        ...
        if solver == 'fixed_iters':
            self.solver = optimal_transport_partial_balanced
        elif solver == 'duality_gap':
            self.solver = optimal_transport_duality_gap
        elif solver == 'partial_balanced':
            self.solver = optimal_transport_partial_balanced
        elif solver == 'weighted':
            self.solver = optimal_transport_with_mask
        elif solver == 'no_norm':
            self.solver = optimal_transport_no_norm
        else:
        ...
        if parameters_from_file is not None:
            config_dict = parse_parameter_file(parameters_from_file)
...
# # Code to create the scaled matrix 
#     def create_scaling_matrix(p0,p1,order_column):

#         return(scaling_matrix)
#     def scale_cost_matrix(cost_matrix=cost_matrix,scaling_matrix=scaling_matrix,alpha=10):
#         """_summary_
#         Takes the input of the default cost_matrix C and scales it according to the 
#         values obtained from diffusion-pseudotime trajectory to create asym
#         Args:
#             cost_matrix (matrix, IxJ): default cost matrix calculated previously.
#             scaling_matrix (matrix,IxJ):scaling_matrix to be created from the pseudotime values.
#             alpha (int, optional): paramter controlling the scaling 
#         """


#         return scaled_cost_matrix
    ...
    def compute_single_transport_map(self, config):
        ...
        if local_pca is not None and local_pca > 0:
            ...
            p0_x, p1_x, pca, mean = compute_pca(p0.X, p1.X, local_pca)
        ...
        if self.growth_average in p1.obs.columns:
            ...
            config['G_avg'] = np.average(config['G'])*np.ones(len(p1.obs))        
        ...
        tmap,pri_df, learned_growth = compute_transport_matrix(solver=self.solver, **config) #added pri_df to output
        ...
        return (anndata.AnnData(tmap, obs, pd.DataFrame(index=p1.obs.index)),pri_df)
