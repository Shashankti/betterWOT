...
def heatmap_tmap(mat,t0,t1,qt,cmap,adata='ot_adata',time_column='time'
                ,title='Transition_matrix',clusters='clusters',
                clusters_colors='clusters_colors',
                sort_dict=sort_dict):
        # Code for visualisation
        vmax = np.quantile(mat,qt)
        # Convert matrix to dataframe with the colnames and rownames derived from the adata
        df = pd.DataFrame(mat,columns=list(adata[(adata.obs[time_column]==t1)].obs[clusters]),
                        index=list(adata[(adata.obs[time_column]==t0)].obs[clusters]))

        # Add helper columns and rows for sorting
        df['row_order'] = df.index.map(sort_dict)
        df.loc['col_order'] = df.columns.map(sort_dict)

        # Sort the DataFrame
        df = df.sort_values('row_order').drop('row_order', axis=1)
        df = df.sort_values('col_order', axis=1).drop('col_order')

        #Assign colors from the uns layer of adata to the cluster types
        node1_labels = df.columns.tolist()
        node1_pal = adata.uns[clusters_colors]
        node1_lut = dict(zip(map(str,adata.obs[clusters].unique().categories.tolist()),
                        adata.uns[clusters_colors]))

        node1_colors = pd.Series(node1_labels,index=df.columns).map(node1_lut)


        node2_labels = df.index.tolist()
        node2_lut = dict(zip(map(str,adata.obs[clusters].unique().categories.tolist())
                        ,adata.uns[clusters_colors]))
        node2_colors = pd.Series(node2_labels,index=df.index).map(node2_lut)

        handles = [Patch(facecolor=node2_lut[name]) for name in node2_lut]


        g = sns.clustermap(df,vmax=vmax,col_cluster=False,row_cluster=False,col_colors=node1_colors,
                row_colors=node2_colors,cmap=cmap)

        # Remove existing row names and ticks
        ...
        # Add custom ticks and row names
        unique_row_names = sorted(list(set(row_names)), key=lambda x: sort_dict[x])
        row_counts = [row_names.count(name) for name in unique_row_names]

        unique_col_names = sorted(list(set(col_names)), key=lambda x: sort_dict[x])
        col_counts = [col_names.count(name) for name in unique_col_names]

        # Calculate the position of the row labels
        row_positions = [sum(row_counts[:i]) + count // 2 for i, count in enumerate(row_counts)]
        col_positions = [sum(col_counts[:i]) + count // 2 for i, count in enumerate(col_counts)]

        # Set new y-tick labels
        g.ax_heatmap.set_yticks(row_positions)
        g.ax_heatmap.set_yticklabels(unique_row_names, rotation=0)
        g.ax_heatmap.set_xticks(col_positions)
        g.ax_heatmap.set_xticklabels(unique_col_names,rotation=90)

        plt.legend(handles,node2_lut,title='Clusters',
